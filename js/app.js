/* 操作系统刷题 PWA（题库：data/ch0.json … ch14.json）
 * 基于 net_pwa 外壳改造。题库暂无答案与解析（a/x 为空）：
 *  - 作答只高亮所选选项并记录进度（correct='na'），不判对错；
 *  - 之后往 data/chN.json 的 a/x 字段填入答案与解析（或用 tools/merge.py 合并），
 *    本应用无需改动即自动恢复判分。
 *
 * 数据按需 fetch + 内存缓存 / 分片渲染 / content-visibility /
 * 事件委托 / 双字倒排搜索索引 / IndexedDB 持久化（localStorage 兜底）/
...
function createCard(q, idx, tabIdx) {
  ...
  // 全题库连续编号 1..457：前缀 = 之前各章题数之和（组内序号 idx+1 保持不变）
  let prefix = 0;
  for (let k = 0; k < tabIdx; k++) prefix += state.chapters[k]?.count || 0;
  const globalNum = prefix + idx + 1;
  ...
  const analysisHTML = (q.x && q.x.trim())
    ? `<div class="analysis"><span class="label">解析：</span>${q.x}</div>`
    : `<div class="analysis"><span class="label">解析：</span><span class="no-x">暂无解析（题库待补充，补充后刷新本页即可显示）</span></div>`;
  card.innerHTML =
    `<div class="q-title"><span class="q-type s">单选</span><span class="num">${idx + 1}. 第${globalNum}题</span> ${q.q}</div>` +
...
function restoreAnswers(container, tabAnswers) {
  Object.entries(tabAnswers).forEach(([qid, record]) => {
    ...
    if (record.correct === 'na') {
      // 暂无答案模式：仅还原所选选项
      card.querySelectorAll('.opt').forEach(o => {
        if (o.dataset.opt === record.userAnswer) o.classList.add('pick');
      });
      return;
    }
    card.querySelectorAll('.opt').forEach(o => {
...
function judgeSingle(card, selectedOpt) {
  ...
  if (!answer) {
    // 暂无答案：记录选择，不判对错
    card.dataset.correct = 'na';
    if (!state.answers[tabIdx]) state.answers[tabIdx] = {};
    state.answers[tabIdx][card.dataset.id] = { userAnswer, correct: 'na' };
    card.querySelectorAll('.opt').forEach(o => { if (o === selectedOpt) o.classList.add('pick'); });
...
function updateScore() {
  ...
  let correctAll = 0, judged = 0, doneAll = 0;
  allCards.forEach(c => {
    if (!c.classList.contains('done')) return;
    doneAll++;
    if (c.dataset.correct === '1') { correctAll++; judged++; }
    else if (c.dataset.correct === '0') judged++;
  });
  const box = $('score-box');
  if (judged > 0) {
    const pct = Math.round(correctAll / judged * 100);
    box.innerHTML = `正确 <span>${correctAll}</span> / <span>${judged}</span> (<span>${pct}%</span>)`;
  } else if (doneAll > 0) {
    box.innerHTML = `已答 <span>${doneAll}</span> / ${allCards.length} · 答案待补充`;
  } else {
    box.textContent = '尚未作答';
  }
...
function updateProgressBar() {
  ...
  $('progress-label').textContent = totalCorrect > 0
    ? `${totalCorrect}✓ / ${totalDone}/${totalQuestions}`
    : `已答 ${totalDone}/${totalQuestions}`;
}
...
function applyFilter() {
  ...
  cards.forEach((card, i) => {
    ...
    switch (filter) {
      ...
      case 'wrong': passes = isDone && card.dataset.correct === '0'; break;
      case 'correct': passes = isDone && card.dataset.correct === '1'; break;
    }
...
function bindEvents() {
  ...
  $('show-all-btn').addEventListener('click', () => {
    ...
    const answerable = cards.filter(c => c.dataset.answer);
    if (answerable.length === 0) {
      showToast('本章题目暂无答案与解析，答案补充后即可使用');
      return;
    }
    if (!confirm(`当前可见还有 ${cards.length} 道未作答（其中 ${answerable.length} 题有答案）。确定展开有答案题目的正确选项吗？`)) return;
    answerable.forEach(card => {
      card.classList.add('done');
    ...
    saveState();
    updateAll();
    if (answerable.length < cards.length) {
      showToast(`已展开 ${answerable.length} 题，其余 ${cards.length - answerable.length} 题暂无答案未计入`);
    }
  });
